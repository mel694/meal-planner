...
export default function SevenDinners() {
  ...
  const generatePlan = async () => {
    ...
    const msgs = ["Finding the perfect recipes...","Matching meals to your tastes...","Building your seven dinners...","Adding macros and shopping list..."];
    ...
    try {
      ...
      if (data.mealPlan) {
        setMealPlan(prev => {
          ...
          const newLines = [...lines.slice(0, dayIndex), ...data.mealPlan.split("\n"), ...lines.slice(end)];
  ...
  const parseMealPlan = (text: string) => {
    ...
    const categories: Record<string, Array<{name:string, sainsburysUrl:string, ocadoUrl:string, tescoUrl:string, asdaUrl:string, morrisonsUrl:string}>> = {};
    let currentCat = "";
    shoppingLines.forEach(line => {
      if (line.startsWith("**") && line.endsWith("**")) {
        ...
        const encoded = encodeURIComponent(itemName.split("(")[0].trim());
        if (currentCat) {
          categories[currentCat].push({
            name: itemName,
            sainsburysUrl: `https://www.sainsburys.co.uk/gol-ui/SearchResults/${encoded}`,
            tescoUrl: `https://www.tesco.com/groceries/en-GB/search?query=${encoded}`,
            asdaUrl: `https://groceries.asda.com/search/${encoded}`,
            morrisonsUrl: `https://groceries.morrisons.com/search?entry=${encoded}`,
            ocadoUrl: `https://www.ocado.com/search?entry=${encoded}`,
          });
        }
      }
    });
  ...
  const renderDayLines = (lines: string[]) => {
    ...
    const flushOl = () => {
      if (olBuffer.length > 0) {
        result.push(<ol key={`ol-${olKey++}`} style={{padding:"0 0 0 20px",margin:"4px 0 8px"}}>{olBuffer}</ol>);
        olBuffer = [];
      }
    };
      ...
      if (line.startsWith("### ")) {
        ...
        result.push(
          <div key={i} style={{display:"flex",flexWrap:"wrap",gap:6,margin:"8px 0"}}>
            {macros.map((m,j) => (
              <span key={j} style={{fontSize:11,padding:"3px 10px",background:"#F0EEFF",border:"1px solid #D0C8FF",borderRadius:100,color:"#5040A0",fontWeight:600}}>{m}</span>
            ))}
          </div>
        );
    ...
    <div style={{minHeight:"100vh",background:"#FFF8F5",fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
      ...
      <div className="no-print" style={{background:"#FF6B35",padding:"14px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:10}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          {step==="plan" && (
            <button onClick={handlePrint} style={{background:"rgba(255,255,255,0.2)",color:"white",border:"1px solid rgba(255,255,255,0.3)",padding:"7px 14px",borderRadius:100,fontSize:12,cursor:"pointer",fontWeight:500}}>🖨️ Print / PDF</button>
          )}
          {step==="plan" && (
            <button onClick={()=>setStep("prefs")} style={{background:"rgba(255,255,255,0.15)",color:"white",border:"1px solid rgba(255,255,255,0.2)",padding:"7px 14px",borderRadius:100,fontSize:12,cursor:"pointer",fontWeight:500}}>← edit</button>
          )}
        </div>
      </div>
        ...
        {step==="prefs" && !loading && (
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
              {["🥗 No UPF","🚫 No pork","⏱️ Quick meals","🛒 Shopping list","📊 Macros included","✨ AI powered"].map((tag,i)=>(
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"#FFF8F5",borderBottom:"1px solid #FFE8DC"}}>
                        ...
                        <div className="no-print" style={{display:"flex",gap:6}}>
                          ...
                          <div style={{position:"relative"}}>
                            <select onChange={(e)=>{if(e.target.value)setDeletedDays(p=>({...p,[day.name]:e.target.value}))}} value={isDeleted||""} style={{fontSize:11,padding:"4px 10px",borderRadius:100,border:"1px solid #FFE8DC",background:"white",color:"#888",cursor:"pointer",appearance:"none",paddingRight:24}}>
              ...
              {Object.entries(categories).map(([cat, items]) => items.length > 0 && (
                <div key={cat}>
                  ...
                  {items.map((item, idx) => {
                    ...
                    const url = (sm as any)[`${sm.id}Url`] || (() => {
                      const encoded = encodeURIComponent(item.name.split("(")[0].trim());
                      return `${sm.search}${encoded}`;
                    })();
